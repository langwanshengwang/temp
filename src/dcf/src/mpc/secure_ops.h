...
struct DpfBitBlockMulMaterial {
    std::vector<u8> a;
    std::vector<osuCrypto::block> b;
    std::vector<osuCrypto::block> c;
    int cursor = 0;
};
...
// Batched additive-to-Boolean decomposition. Each row in `output` is
// MSB-first and corresponds to one arithmetic input share. The carry circuit
// is identical to BitDec, but every bit position is processed across all
// inputs in one batch.
void BitDecBatch(int party_id, const GroupElement* input, int size, int bits,
